...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
    ALTER TABLE "tnf"."payload_locked_documents_rels"
      DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_consultation_questions_fk";
    DROP INDEX IF EXISTS "tnf"."payload_locked_documents_rels_consultation_questions_id_idx";
    ALTER TABLE "tnf"."payload_locked_documents_rels"
      DROP COLUMN IF EXISTS "consultation_questions_id";

    ALTER TABLE "tnf"."form_submissions" DROP COLUMN IF EXISTS "question";

    DROP TABLE IF EXISTS "tnf"."consultation_questions";
    DROP TYPE IF EXISTS "tnf"."enum_consultation_questions_status";
    DROP TYPE IF EXISTS "tnf"."enum_consultation_questions_form";
  `);
}
